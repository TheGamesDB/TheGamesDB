...
<h1>Missing Series Banners (Top 100)</h1>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center" id="listtable">
		...
		<?php	## Display series
			$count = 0;
			$query = "SELECT * FROM tvseries WHERE bannerrequest>0 AND disabled!='Yes' ORDER BY bannerrequest DESC LIMIT 100";
...
<h1>Missing Season Banners (Top 100)</h1>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center" id="listtable">
		...
		<?php	## Display series
			$count = 0;
			$query = "SELECT *, (SELECT SeriesName FROM tvseries WHERE id=tvseasons.seriesid ) AS SeriesName FROM tvseasons WHERE bannerrequest>0 AND season!=0 ORDER BY bannerrequest DESC LIMIT 100";
			$result = mysql_query($query) or die('Query failed: ' . mysql_error());
			while ($db = mysql_fetch_object($result)) {
				if ($class == 'odd')  {  $class = 'even';  }  else  {  $class = 'odd';  }
